...
/**
 * Krok, kterému dané procento odpovídá. Procento a krok jsou jedna věc
 * (viz komentář u REQUEST_STATUSES) — tohle je jediné místo, kde se jedno
 * převádí na druhé.
 *
 * @param {number} progress
 * @returns {Object|null} definice kroku, nebo null u hodnoty mimo kroky
 */
function _requestStepForProgress_(progress) {
	return REQUEST_STATUSES.filter((s) => s.progress === Number(progress))[0] || null;
}

/**
 * Ověří poslané procento pokroku a vrátí ho.
 *
 * Musí odpovídat některému kroku — posuvník v UI jiné hodnoty nenabízí,
 * takže cokoli jiného by se sice uložilo, ale nešlo by to zobrazit jako
 * krok ani znovu trefit.
 *
 * @param {number} requestedProgress
 * @returns {number} 0–100, vždy násobek REQUEST_PROGRESS_STEP
 */
function _requestProgressFor_(requestedProgress) {
	...
	if (!_requestStepForProgress_(value)) {
...
function _publicRequest_(row, user, settings, authorCache, commentCounts) {
	// Zdroj pravdy je PROCENTO. Sloupec `status` se dál zapisuje, ať je list
	// čitelný i bez appky, ale při čtení se ignoruje — kdyby se ty dvě
	// hodnoty kdy rozešly (ruční zásah v listu, starší řádek), rozhoduje
	// procento a krok se z něj dopočítá.
	const def = _requestStepForProgress_(Number(row.progress || 0)) || REQUEST_STATUSES[0];
 ...
 *
 * Průběh se tudy NEMĚNÍ (ani při zakládání) — na to je apiSetRequestStep
 * s vlastní kontrolou práv. Nový požadavek proto vždy začíná prvním krokem
 * z REQUEST_STATUSES.
 *
...
/**
 * Posune požadavek na jiný krok průběhu.
 *
 * Přijímá `progress` (posuvník a tlačítka -/+) NEBO `step` (klik přímo na
 * krok) — obojí je tatáž hodnota vyjádřená jinak, takže se uvnitř hned
 * převede na procento a dál se pracuje jen s ním. Žádná pravidla
 * o vzájemném dopočítávání stavu a procenta tu nejsou, protože není co
 * dopočítávat: krok JE procento.
 *
 * Oddělený endpoint od apiSaveRequest proto, že tohle je jediná část
 * požadavku s jiným pravidlem než „vlastník nebo správce" — viz
 * canManageRequestStatus_ v 30_auth.js.
 *
 * @param {Object} payload  { id, progress?, step? } — aspoň jedno z nich
 * @returns {Object} uložený požadavek ve tvaru pro klienta
 */
function apiSetRequestStep(payload) {
	return guard_(PERM_KEYS.CALENDAR_READ, (user) => {
		const data = payload || {};
		...
		if (!canManageRequestStatus_(user, settings)) {
			...
			throw userError_(
				'Průběh požadavku smí měnit jen uživatel s umístěním a pozicí ' + wanted +
		...
		if (data.step !== undefined && data.step !== null && data.step !== '') {
			const wantedStep = _requestStatusDef_(data.step);
			if (!wantedStep) throw userError_('Neznámý krok průběhu.');
			progress = wantedStep.progress;
		} else {
